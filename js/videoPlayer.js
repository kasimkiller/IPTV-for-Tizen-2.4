    ...
    function create(config) {
        ...
        function toggleFullscreen() {
            if (!isFullscreen) {
                ...
                if (headerEl) {
                    headerEl.classList.add(FULLSCREEN_CLASS);
                }
                ...
                // 退出沉浸：显示头部
                var headerElOff = document.querySelector('.main-header');
                if (headerElOff) {
                    headerElOff.classList.remove(FULLSCREEN_CLASS);
                }
        ...
        function initialize(url) {
            try {
                ...
                webapis.avplay.setDisplayMethod('PLAYER_DISPLAY_MODE_AUTO_ASPECT_RATIO');
                // 根据三星AVPlay API官方文档设置缓冲参数
                webapis.avplay.setBufferingParam("PLAYER_BUFFER_FOR_PLAY","PLAYER_BUFFER_SIZE_IN_SECOND", 15);
